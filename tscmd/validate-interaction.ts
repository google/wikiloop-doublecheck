...
async function fixFeed() {
  const ret = await Interaction.updateMany({ feed: { $exists: false } }, { feed: 'index' });
...
async function fixUserGaId() {
  ...
  const ret = await Interaction.updateMany({
    userGaId: { $exists: false },
    gaId: { $exists: true },
...
async function fixJudgement() {
  const ret = await Interaction.deleteMany({ judgement: { $exists: false } });
...
async function printStatus() {
  ...
  const ret = await getInvalidCount();
...
async function validateAll() {
  ...
  const invalid = (await Interaction.find()).map((i) => {
    const e = i.validateSync();
    return e;
  }).filter((e) => e);
  const count = {
    total: 0,
...
async function main() {
  ...
  await printStatus();
  const deleteRet = await Interaction.deleteMany(
    {
      $or: [
        {
          $and: [
  ...
  const errorInteractions = (await Promise.all(docs.map(async (i:InteractionDoc) => {
    try {
      console.log(`Start doc ${i}`);
      const hardRecoverFields = i.get('_meta.HardRecoveredFields') || [];
      if (!i.feed) {
        i.feed = 'index';
      }
      if (!i.userGaId) {
      ...
      if (!i.timestamp) {
        const rcTsRaw = parseFloat(i.get('recentChange.timestamp'));
