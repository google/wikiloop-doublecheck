...
const mainTraverseLinkGraph = async function ()
{
  ...
  const HOP_LIMIT = 2;
  let toVisit = [];
  toVisit.push(...seed.map(article => [article, HOP_LIMIT]));
  seed.forEach(article => touched.add(article));
  ...
  while (toVisit.length > 0) {
    console.log(`visited ${visited.size}, toVisit = ${toVisit.length}, touched ${touched.size}, touch/visited = ${touched.size / visited.size}, toVisit/visited = ${toVisit.length/visited.size}`);
    let curr = toVisit.shift();
    let ttl = curr[1];
    let article = curr[0];
    if (visited.has(article)) {
      console.log(`Skipping already visited ${article}`);
      continue;
    }
    else {
      console.log(`Visiting article="${article}" at ttl=${ttl}...`);
      visited.add(article);
      let result = await MwActionApiClient.getLinkChildren('enwiki', article);
      if (ttl >= 1) {
        let filtered = result
          .filter(article => !touched.has(article))
          .map(article => [article, ttl-1]);
        console.log(`Adding ${filtered.length} from ${article}`);
        toVisit.push(...filtered);
        filtered.forEach(article => touched.add(article));
        await filtered.forEach(async s => {
          console.log(`${outputCount} Writing article=${s[0]} to file`);
          await appendFile('output.txt', s[0] + "\n");
          outputCount++;
        });
      }
    }
  }
  await visited.forEach(async (s) => {
    console.log(`${outputCount} Writing article=${s} to file`);
    await appendFile('output.txt', s);
    outputCount++;
  });
}
