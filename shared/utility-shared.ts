...
export const fetchDiffWithWikiRevId = async function(wikiRevId, $axios) {
  ...
  const diffApiUrl = `/api/diff/${wiki}:${revId}`;
  const diffJson = await $axios.$get(diffApiUrl);
  return diffJson;
};
...
export const fetchRevisionPanelItem = async function(
  wikiRevId,
  $axios,
): Promise<RevisionPanelItem> {
  const [revision, diff] = await Promise.all([
    await $axios.$get(`/api/revision/${wikiRevId}`),
    await $axios.$get(`/api/diff/${wikiRevId}`),
  ]);
  const diffHtml = diff?.compare['*'] || '';
  const diffMetadata = diff?.compare.diffMetadata || null;
