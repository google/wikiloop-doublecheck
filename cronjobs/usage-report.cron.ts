    ...
    public static usageReport = async function(frequency) {
      ...
      if (envAssert('REPORT_RECEIVER') && process.env.REPORT_RECEIVER.length > 0) {
        if (envAssert('MAIL_SENDER_USERNAME') && envAssert('MAIL_SENDER_PASSWORD')) {
          ...
          const formatedDate/** format: YYYY-MM-DD */ = new Date().toISOString().split('T')[0];
          const report = await require('../server/routes/stats').getChampion(frequencyToNumDaysMap[frequency], formatedDate, 'enwiki');
          const json2html = require('node-json2html');
          const html = json2html.transform(
            report, {
              '<>': 'tr',
              'html': [
