...
describe('REMOTE MwActionApiClient2.fetchRevisionInfo', () => {
  ...
  test('should return null without error if given a revId never exists. REMOTE', async () => {
    const wiki = 'enwiki';
    const info = await mwapi2.fetchRevisionInfo(wiki, 1234567890123456);
    expect(info).toBe(null);
  });

  test('should return revision full information if given a revId which is DELETED. REMOTE', async () => {
    const wiki = 'enwiki';
    const info = await mwapi2.fetchRevisionInfo(wiki, 907978564);
    expect(info.wiki).toBe('enwiki');
  ...
  test('should handle a page creation', async () => {
    const wiki = 'enwiki';
    const revId = 797951879; // a page creation for page "Wei Dai": https://en.wikipedia.org/w/index.php?title=Wei_Dai&oldid=718234275
    const info = await mwapi2.fetchRevisionInfo(wiki, revId);
    expect(info.wiki).toBe('enwiki');
  ...
  test('should handle a page protection', async () => {
    const wiki = 'enwiki';
    const revId = 991836653; // a PageProtection revision
    const info = await mwapi2.fetchRevisionInfo(wiki, revId);
    expect(info.wiki).toBe('enwiki');
  ...
  test('should handle a revId which is a PageMoveTo', async () => {
    const wiki = 'enwiki';
    const revId = 991839618; // a PageMoveTo revision
    const info = await mwapi2.fetchRevisionInfo(wiki, revId);
    expect(info.wiki).toBe('enwiki');
  ...
  test('should handle a revId which is a PageMoveFrom', async () => {
    const wiki = 'enwiki';
    const revId = 991839619; // a PageMoveFrom revision
    const info = await mwapi2.fetchRevisionInfo(wiki, revId);
    expect(info.wiki).toBe('enwiki');
...
describe('REMOTE MwActionApiClient2.fetchDiff', () => {
  expect.extend({
    toMatchHtml(received, expected) {
      ...
      const diff = htmlDiffer.diffHtml(received, expected);
  ...
  test('should take a given revId without prevRevId and return diff content in HTML in the exact same output we got.', async () => {
    ...
    const expectedHtml = testData.diff_from_904396518_to_904395753;

    const wiki = 'enwiki';
    const fetchedHtml = await mwapi2.fetchDiff(wiki, 904396518);
  ...
  test('should handle a page creation', async () => {
    // This test will fail when MediaWiki change how diff is generating.
    const expectedHtml = testData.diff_from_0_to_797951879;

    const wiki = 'enwiki';
    const revId = 797951879; // page creation;
    const fetchedHtml = await mwapi2.fetchDiff(wiki, revId);
