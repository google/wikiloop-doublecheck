...
export default {
  ...
  computed: {
    ...
    currentOres: {
      get() {
        let ores = this.$store.state.revisions.wikiRevIdToMeta[
          this.currentWikiRevId
        ].ores
        return ores
      }
  ...
  async beforeMount() {
    ...
    if (
      ['enwiki', 'testwiki'].indexOf(this.$store.state.wiki) >= 0 &&
      dice < threshold
    ) {
    ...
    window.addEventListener('keyup', async e => {
      switch (e.code) {
        ...
        case 'KeyR':
          ...
          break
        case 'ArrowRight':
          if (this.$refs.revisionCard && this.$refs.revisionCard.myJudgement)
            this.showNext()
