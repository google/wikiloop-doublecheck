    ...
    <div class="card-body d-flex flex-column small-screen-padding">
      <h2 class="mb-4">
        <div class="d-flex">
          ...
          <div class="ml-2">
            <a :href="permUrl"><i class="fas fa-link" /></a>
          </div>
        </div>
      </h2>
      <div class="wldc-revinfo-area text-muted mb-2 mb-sm-2 mb-xs-1">
        <div class="wldc-revinfo wldc-revinfo-revid">
          <i class="fas fa-external-link-alt mr-2" />{{ item.revId }}
        </div>
        <div class="wldc-revinfo wldc-revinfo-timeago">
          <i class="fas fa-pen mr-2" /><timeago :datetime="timeString" :auto-update="60" :locale="$i18n.locale" />
        </div>
        <div class="wldc-revinfo wldc-revinfo-author">
          <i class="fas fa-user mr-2" /><a :href="authorUrl" target="_blank">{{ item.author }}</a>
        </div>
        <div v-if="feedName" class="wldc-revinfo wldc-revinfo-feed">
          <i class="fas fa-faucet mr-2" />{{ feedName }}
        </div>
      </div>
      <div class="w-100 pl-sm-0 mb-2 flex-grow-1 flex-shrink-1 overflow-hidden justify-content-start">
        <template v-if="item.diffHtml">
          <diff-box
            class="wldc-rev-panel-diff-container"
            :diff-content="item.diffHtml"
            :wiki-rev-id="`${item.wiki}:${item.revId}`"
            :diff-metadata="item.diffMetadata"
          />
        </template>
        <template v-else>
          <loading-indicator />
      ...
      <div class="card-text w-100 pl-sm-0 mb-3">
        <template v-if="item.summary">
          <h3 class="wldc-revinfo-h3">
            {{ $t('Label-EditSummary') }}
          </h3>
          <span class="wldc-summary-content">{{ item.summary }}</span>
        </template>
        <template v-else>
          <h5 class="text-danger w-100">
            {{ $t('Message-ThereIsNoEditSummary') }}
          </h5>
        </template>
      </div>
    </div>
