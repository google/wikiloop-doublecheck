<template>
  <div class="w-100">
    <div class="card shadow" v-if="!loading && notice">
      <div class="card-body">
        <!-- TODO add internationalization for message. -->
        <span>{{ notice.defaultMessage }}</span>
      </div>
      <div class="card-footer">
        <a target="_blank" class="btn btn-primary btn-sm float-right"
          :href="notice.url"><i class="fas fa-chevron-right"></i> {{$t('Button-Go')}}</a>
        <button @click="dismiss()" class="btn btn-secondary btn-sm float-right mr-2">
...
export default class NoticeBanner extends Vue {
  ...
  load = async function() {
    ...
    if (this.$store.state.user?.profile?.displayName)
      searchParams.set('wikiUserName', this.$store.state.user?.profile?.displayName);
    else searchParams.set('userGaId', this.$cookiez.get('_ga'));
    let notices = await this.$axios.$get(`/api/notice/list?${searchParams.toString()}`);
    if (notices.length >0) {
